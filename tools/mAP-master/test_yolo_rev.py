...
parser.add_argument(
    '--predict_txt_dir',
    type=str,
    help='predict txt file for mAP calculation',
    default = 'predict_txt')

def write_txt(saveDir, imagename, bounding_boxes):
    save_path = os.path.join(saveDir, os.path.splitext(imagename)[0] + ".txt")
    with open(save_path, 'wt') as t:
        for i, boxes in enumerate(bounding_boxes):
            info = [x for x in map(lambda x: str(x), boxes)]
            info = ' '.join(info)
...
def _main(args):
    ...
    predict_txt_path = os.path.expanduser(args.predict_txt_dir)

    if not os.path.exists(predict_txt_path):
        os.mkdir(predict_txt_path)
    ...
    for image_file in os.listdir(test_path):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            ##### try to output the prediction txt file
            detected_txt.append([predicted_class, score, left, top, right, bottom])
            
            detected.append([label, (left, top, right, bottom)])

        write_txt(predict_txt_path, image_file, detected_txt)
