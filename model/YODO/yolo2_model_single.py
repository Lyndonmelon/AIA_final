...
if __name__ == '__main__':
    weight_file = "./model/trained_stage_1_0110_6997.h5"
    config_file = "./config/yolo_anchors.txt"
    classes_file = "./config/class_list.txt"

    class_names = load_classes(classes_file)

    sess = K.get_session()
    yolo_model, ts_parm = build_model(weight_file, config_file, class_names)

    image = Image.open(os.path.join('./test_images/', 'HiLife_img_48.jpg'))
    ...
    for i in range(10):
        btime = time.time()
        result = detect_object(sess, yolo_model, ts_parm, class_names, image)
        etime = time.time()

        print('\tresult:', result)
        print('\taction{} elapse time: {}'.format(i+1, etime-btime))
